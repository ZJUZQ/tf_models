...
parser.add_argument('--eval_dir', type=str, default='/cifar10/cifar10_eval',
                    help='Directory where to write event logs.')

parser.add_argument('--eval_data', type=str, default='test',
                    help='Either `test` or `train_eval`.')

parser.add_argument('--checkpoint_dir', type=str, default='/cifar/cifar10_train',
...
def eval_once(saver, summary_writer, top_k_op, summary_op):
  ...
  with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state(FLAGS.checkpoint_dir)  # Returns CheckpointState proto from the "checkpoint" file
    ...
    try:
      ...
      for qr in tf.get_collection(tf.GraphKeys.QUEUE_RUNNERS):
        threads.extend(qr.create_threads(sess, coord=coord, daemon=True,  # Create threads to run the enqueue ops for the given session
                                         start=True))
      ...
      summary = tf.Summary()
      summary.ParseFromString(sess.run(summary_op)) # Parse serialized protocol buffer data into this message
    ...
    coord.join(threads, stop_grace_period_secs=10)  # Wait for threads to terminate
